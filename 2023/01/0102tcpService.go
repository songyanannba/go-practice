...
func main() {
	...
	for {
		...
		for {
			writeString, err := writer.WriteString(time.Now().Format("2006-01-02结构体排序sort.Slice 15:04:05") + "\n")
			writer.Flush()
			if err != nil {
		...
		//client.Write([]byte(time.Now().Format("2006-01-02结构体排序sort.Slice 15:04:05")))
