...
var (
	clusterService = ServiceGroupApp.ClusterService
)

// 认证
func (m MetricsService) Auth(clusterObj model.Cluster) (httpClient http.Client) {
...
func (m MetricsService) Health(clusterId int) (url string, httpClient http.Client, err error) {
	clusterObj, err := clusterService.GetGlusterById(clusterId)
	if err != nil {
	...
	if err != nil {
		global.GVA_LOG.Error("prometheus service ready request failed", zap.Any("err", err))
		return
	}
	...
	if readyResp.StatusCode != http.StatusOK {
		global.GVA_LOG.Error("prometheus resp no ready", zap.Any("err", err))
		return
	}
...
func (m MetricsService) GetMetrics(mt model.MetricsQuery) (t map[string]*model.PrometheusQueryResp, err error) {
	step := 60
	end := time.Now().Unix()
	start := end - 3600
	...
	for i := 0; i < e.NumField(); i++ {
		...
		go func(i int) {
			...
			if prometheusQueries == nil {
				global.GVA_LOG.Error("prometheusQueries nill failed", zap.Any("err", err))
				return
			}
			...
			if err != nil {
				global.GVA_LOG.Error("Prometheus health failed", zap.Any("err", err))
				return
			}

			//http Get请求 Prometheus接口
			promql := url.QueryEscape(prometheusQueries.GetValueByField(fName))
			global.GVA_LOG.Info(fmt.Sprintf("promql: %s ", prometheusQueries.GetValueByField(fName)))
			...
			if err != nil {
				global.GVA_LOG.Error("request metrics data failed", zap.Any("err", err))
				return
			}
			...
			if err != nil {
				global.GVA_LOG.Error("read response body failed", zap.Any("err", err))
				return
			}

			var data model.PrometheusQueryResp
			if err := json.Unmarshal(body, &data); err != nil {
				global.GVA_LOG.Error("unmarshal response body to models.PrometheusQueryResp failed", zap.Any("err", err))
				return
			}
