...
type {{.StructName}} struct {
      ...
      {{.FieldName}}  uint8 `json:"{{.FieldJson}}" form:"{{.FieldJson}}" gorm:"column:{{.ColumnName}};default:1;comment:{{.Comment}};size:8;"`
            {{- else if eq .FieldType "int" }}
      {{.FieldName}}  int `json:"{{.FieldJson}}" form:"{{.FieldJson}}" gorm:"column:{{.ColumnName}};default:0;comment:{{.Comment}};size:32;"`
